...
function effectScroll(){
    $(window).on("load",function() {
        ...
        $(".aos-init").each(function() {
            var objectBottom = $(this).offset().top + 150,
                reScroll = $(this).attr("rel");
            if (objectBottom < windowBottom) {
                $(this).addClass('aos-animate');
            } else if (objectBottom >= windowBottom && reScroll == "reScroll") {
...
function scrollTo($param){
    $param.on('click', function (e) {
        var link = $(this).attr('rel'),
            h = parseInt($('#header').outerHeight() - 1);
        if (link == "header"){
            $('html,body').animate({
                scrollTop: 0
            }, 1000);
...
function scrollElement(){
    var a = $(location).attr('search'),
        b = $(location).attr('href').lastIndexOf("/") + 1,
        c = $(location).attr('href').substr(b).split(/[.?&#]+/)[0];
    switch(a) {
        case "?about":
        $('html, body').animate({
